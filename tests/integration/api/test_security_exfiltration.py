...
def api_security_data(app, initial_data):
    ...
    with app.app_context():
        ...
        account_scope_key = ApiKey(
            desc='account-only-key',
            role_name='User',
            accounts=[account],
        )
        account_scope_plain_key = account_scope_key.plain_key
        account_scope_key.create()
        ...
        data = {
            ...
            'account_scope_key_headers': apikey_headers(
                account_scope_plain_key),
            'administrator_key_headers': apikey_headers(
                administrator_plain_key),
            'scoped_key_id': scoped_key.id,
            'account_scope_key_id': account_scope_key.id,
...
def test_basic_auth_user_zone_list_respects_direct_and_account_grants(
        app, client, api_security_data):
    unrelated_domain_name = 'security-unrelated-basic.example'
    with app.app_context():
        account = Account.query.filter_by(
            name=api_security_data['private_account_name']).one()
        account.grant_privileges([app.config['TEST_USER']])
        unrelated_domain = Domain(name=unrelated_domain_name)
        db.session.add(unrelated_domain)
        db.session.commit()
    ...
    try:
        ...
        assert response.status_code == 200
        zone_names = {zone['name'] for zone in response.get_json()}
        assert api_security_data['allowed_domain_name'] in zone_names
        assert api_security_data['private_domain_name'] in zone_names
        assert unrelated_domain_name not in zone_names
    finally:
        with app.app_context():
            account = Account.query.filter_by(
                name=api_security_data['private_account_name']).one()
            account.grant_privileges([])
            unrelated_domain = Domain.query.filter_by(
                name=unrelated_domain_name).first()
            if unrelated_domain is not None:
                db.session.delete(unrelated_domain)
            db.session.commit()


def test_basic_auth_user_zone_list_excludes_unrelated_account(
        app, client, api_security_data):
    account_name = 'security-unrelated-account'
    domain_name = 'security-unrelated-account.example'
    with app.app_context():
    ...
    try:
        ...
        assert response.status_code == 200
        assert domain_name not in {
            zone['name'] for zone in response.get_json()
        }
    finally:
        with app.app_context():
            ...
            if domain is not None:
                db.session.delete(domain)
            account = Account.query.filter_by(
                name=Account.sanitize_name(account_name)).first()
            if account is not None:
                db.session.delete(account)
            db.session.commit()
...
def test_user_cannot_create_administrator_apikey(
        app, client, api_security_data):
    response = client.post(
        '/api/v1/pdnsadmin/apikeys',
        headers=api_security_data['ordinary_headers'],
        json={
            'description': 'forbidden-user-administrator-key',
            'role': 'Administrator',
        },
    )
    ...
    with app.app_context():
        assert ApiKey.query.filter_by(
            description='forbidden-user-administrator-key').first() is None


@pytest.mark.parametrize('payload', [
    pytest.param({
        'description': 'forbidden-user-account-key',
        'role': 'User',
        'accounts': ['securityprivateaccount'],
    }, id='account-only'),
    pytest.param({
        'description': 'forbidden-user-account-override-key',
        'role': 'User',
        'domains': ['security-allowed.example'],
        'accounts': ['securityprivateaccount'],
    }, id='mixed-domain-and-account'),
])
def test_user_cannot_assign_account_scope_to_apikey(
        app, client, api_security_data, payload):
    payload = {
        **payload,
        'accounts': [api_security_data['private_account_name']],
    }
    ...
    assert response.status_code == 401
    assert 'assign accounts' in response.get_json()['msg']
    with app.app_context():
        assert ApiKey.query.filter_by(
            description=payload['description']).first() is None
...
def test_account_scoped_user_apikey_lists_only_account_zones(
        client, api_security_data, monkeypatch):
    monkeypatch.setattr(
        'powerdnsadmin.routes.api.helper.forward_request',
        lambda: ForwardResponse(content=json.dumps([
            {'name': api_security_data['allowed_domain_name'] + '.'},
            {'name': api_security_data['private_domain_name'] + '.'},
            {'name': 'security-unrelated.example.'},
        ]).encode('utf-8')),
    )

    response = client.get(
        '/api/v1/servers/localhost/zones',
        headers=api_security_data['account_scope_key_headers'],
    )
...
def test_account_scoped_user_apikey_can_access_account_zone(
        client, api_security_data, monkeypatch):
    monkeypatch.setattr(
        'powerdnsadmin.routes.api.helper.forward_request',
        lambda: ForwardResponse(content=b'[]'),
...
def test_account_scoped_user_apikey_cannot_access_unrelated_zone(
        client, api_security_data, monkeypatch):
    monkeypatch.setattr(
        'powerdnsadmin.routes.api.helper.forward_request',
        lambda: pytest.fail('unrelated zone request must not be forwarded'),
    )
...
def test_user_apikey_cannot_use_any_zone_subpath_method_outside_scope(
        method, client, api_security_data, monkeypatch):
    monkeypatch.setattr(
        'powerdnsadmin.routes.api.helper.forward_request',
        lambda: pytest.fail('unrelated zone request must not be forwarded'),
    )
...
def test_operator_can_create_account_scoped_user_apikey(
        app, client, api_security_data):
    response = client.post(
        '/api/v1/pdnsadmin/apikeys',
        headers=api_security_data['operator_headers'],
        json={
            'description': 'operator-account-key',
            'role': 'User',
            'accounts': [api_security_data['private_account_name']],
        },
    )
    ...
    assert response.status_code == 201
    assert response.get_json()['accounts'][0]['name'] == \
        api_security_data['private_account_name']
    with app.app_context():
        key = ApiKey.query.filter_by(
            description='operator-account-key').one()
        assert [account.name for account in key.accounts] == [
            api_security_data['private_account_name']
        ]


def test_operator_apikey_bypasses_zone_scope(
        client, api_security_data, monkeypatch):
    response = client.post(
        '/api/v1/pdnsadmin/apikeys',
        headers=api_security_data['operator_headers'],
    ...
    response = client.get(
        '/api/v1/servers/localhost/zones/{}/metadata'.format(
            api_security_data['private_domain_name']),
        headers={'X-API-KEY': response.get_json()['plain_key']},
    )

    assert response.status_code == 200


def test_administrator_apikey_bypasses_zone_scope(
        client, api_security_data, monkeypatch):
    monkeypatch.setattr(
        'powerdnsadmin.routes.api.helper.forward_request',
        lambda: ForwardResponse(content=b'[]'),
...
def test_user_cannot_add_account_scope_to_existing_apikey(
        app, client, api_security_data):
    response = client.put(
        '/api/v1/pdnsadmin/apikeys/{}'.format(
            api_security_data['scoped_key_id']),
        headers=api_security_data['ordinary_headers'],
        json={'accounts': [api_security_data['private_account_name']]},
    )

    assert response.status_code == 401
    with app.app_context():
        key = db.session.get(ApiKey, api_security_data['scoped_key_id'])
        assert [domain.name for domain in key.domains] == [
            api_security_data['allowed_domain_name']
        ]
        assert key.accounts == []


def test_administrator_can_add_account_scope_to_existing_apikey(
        app, client, api_security_data, basic_auth_admin_headers):
    response = client.put(
        '/api/v1/pdnsadmin/apikeys/{}'.format(
            api_security_data['scoped_key_id']),
        headers=basic_auth_admin_headers,
...
def test_operator_can_replace_account_scope_on_existing_apikey(
        app, client, api_security_data):
    response = client.put(
        '/api/v1/pdnsadmin/apikeys/{}'.format(
            api_security_data['mixed_scope_key_id']),
        headers=api_security_data['operator_headers'],
        json={
            'domains': [api_security_data['allowed_domain_name']],
            'accounts': [],
        },
    )
    ...
    assert response.status_code == 204
    with app.app_context():
        key = db.session.get(ApiKey, api_security_data['mixed_scope_key_id'])
        assert [domain.name for domain in key.domains] == [
            api_security_data['allowed_domain_name']
        ]
        assert key.accounts == []
